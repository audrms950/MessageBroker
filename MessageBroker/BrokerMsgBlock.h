...
public: /* 환경 상수*/
    ...
    const long long time_to_live_sec;
    const long long created_time; /* 블록 생성 시간 기록용 */
    const unsigned int start_msg_offset;

public:
    BrokerMsgBlock(unsigned int start_offset, long long current_time, unsigned int max_ref_count = 1, unsigned int max_buffer_size = 1024 * 8192, long long time_to_live_sec = 3600 * 24 * 31);

    bool isDeleteAble(long long cur_call_time);
